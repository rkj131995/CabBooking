...
export class CabService implements OnInit{
  ...
  //For getting all my cab mates
  getMyCabMates(id:any):any
  {
    return this.http.get(this.baseUri+'/servers/getMyCabMates?bookingId='+id).map(res => res.json());

  }
  getBookings():any
  {
    return this.http.get('http://blrlw6631.asia.manh.com:9090/servers/getTodaysBookings').map(this.extractData).catch(this.handleError); 
  }  
